...
- PWA としての Manifest / Service Worker / installable 対応は、主要画面の本物データ化後の Slice 26 で扱う
...
- トレンド分析 / オシレーター分析の区分を揃えた表示整理
- 一目均衡表 / RSI / ストキャスティクス / ボリンジャーバンド / MACD の判定を使った確認メモ
- 保有登録 / 編集画面への実データ導線
...
- 25MA / 75MA / 一目均衡表 / ボリンジャーバンド / MACD / 直近高値ライン / 買値 / 損切りラインを個別に表示 / 非表示切り替えできる
- RSI / ストキャスティクスを個別に表示 / 非表示切り替えできる
- トレンド分析とオシレーター分析の区分が、チェックボックス / 分析表示 / 確認メモで揃っている
- 一目均衡表 / RSI / ストキャスティクス / ボリンジャーバンド / MACD の判定が銘柄詳細で確認できる
- 確認メモが一目均衡表 / RSI / ストキャスティクス / ボリンジャーバンド / MACD の判定を反映している
...
- トレンド分析 / オシレーター分析の区分どおりに表示が揃っていることを確認
...
- 銘柄詳細 / 検索 / ホームでのウォッチ導線
- 銘柄詳細 / 検索 / ホームでの保有中表示
- 保有入力 / 編集画面での最近見た銘柄 / ウォッチ銘柄 / 保有導線の整理
- Supabase を正本、IndexedDB を表示高速化用キャッシュとした最近見た銘柄 / ウォッチ銘柄同期
- 最近見た銘柄件数の上限整理
- ウォッチ銘柄件数の上限整理
- 銘柄詳細からホーム / 検索へ戻った時にも導線が崩れない状態整理
...
### 完了条件
- 銘柄詳細を開くと最近見た銘柄へ反映される
- ウォッチ銘柄の追加 / 削除ができる
- スマホ / PC の起動時同期で最近見た銘柄 / ウォッチ銘柄が揃う
- ホームと検索で最近見た銘柄 / ウォッチ銘柄が表示できる
- 保有中の銘柄が検索結果や銘柄詳細で分かる
- 保有入力 / 編集画面から見ても、最近見た銘柄 / ウォッチ銘柄 / 保有導線が不自然なくつながっている
- モバイル幅とデスクトップ幅で最近見た銘柄 / ウォッチ銘柄の表示が破綻しない

### テスト / 確認観点
- `pnpm lint`
- `pnpm typecheck`
- `pnpm test`
- 最近見た銘柄 / ウォッチ銘柄保存ロジックのユニットテストを追加
- 銘柄詳細 → ホーム / 検索への導線確認
- 保有入力 / 編集画面との導線確認
- ホーム / 検索での表示確認
...
- チャート設定の保存先実装
- チャート設定画面の実装
- 銘柄詳細チャートが保存済み設定を使って 25MA / 75MA / 直近高値ライン / 買値 / 損切りラインを表示できる状態
- 損切りラインの計算に使う設定値の入力導線
- 将来のチャート設定追加に耐える最小限の設定構造整理
...
- チャート設定をユーザーが変更できる
- 変更した設定が端末内で保持される
- 銘柄詳細チャートが保存済み設定を使って表示される
- 既存の 25MA / 75MA / 直近高値ライン / 買値 / 損切りライン表示が設定変更で破綻しない
- モバイル幅とデスクトップ幅で設定画面と設定反映後のチャート表示が成立する
...
- `pnpm test`
- 設定保存ロジックのユニットテストを追加
- モバイル幅 / デスクトップ幅での設定画面確認
- 設定変更後に銘柄詳細へ戻って反映される確認
