...
- Mac worker のセットアップは `docs/setup/mac-launchd-import-worker.md` を参照する
...
- 取得対象は日本株を主軸に、米国株と香港株も同じ日足取り込み基盤で扱う
- 投資対象商品は MVP では 株式 / ETF / 為替 を扱い、`hkex reits` は取り込み対象から外す
...
- `hkex reits` は存在確認済みだが、MVP では取り込み対象から外す
- 外貨建て保有の JPY 換算用に `USDJPY` / `HKDJPY` の為替日次も扱う
