...
- 画面やドメイン計算で使う商品種別は `securityType` として正規化し、当面は `stock` / `etf` / `currency` を基本に扱う
...
- 当面の取り込み対象カテゴリは `stocks` / `etfs` / `currencies` を基本とし、`lse stocks intl` と `hkex reits` は MVP の取り込み対象から外す
...
- `/data/daily/uk/lse stocks intl` は意味づけ未確定のため、当面は MVP の取り込み対象から外す
- 各カテゴリ配下は、直下に `.txt` があるケースと、`1/`, `2/` などの下位フォルダ配下に `.txt` があるケースの両方を許容する
- ヘッダーは日本 / 為替と同じ CSV 形式
...
- `/data/daily/hk/hkex reits` は存在確認済みだが、当面は MVP の取り込み対象から外す
- 各カテゴリ配下は、直下に `.txt` があるケースと、`1/`, `2/` などの下位フォルダ配下に `.txt` があるケースの両方を許容する
- ヘッダーは日本 / 為替と同じ CSV 形式
...
- `lse stocks intl` と `hkex reits` は存在確認済みだが、当面は自動判定対象にも取り込み対象にも含めない
