...
1. bulk 取得不可、bulk 内の個別銘柄欠損、または cron 失敗マーク付き銘柄を検知
2. 対象銘柄の `sourceSymbol` を使って Stooq 個別 CSV endpoint に問い合わせ
3. 取得できた日足を通常の価格データと同じ形式に正規化
4. Supabase の最新価格 / 指標 / 失敗状態を更新
5. 必要に応じて R2 の正規化済み価格履歴へ次回全更新で反映
6. 取得できない場合は unsupported / noData / failed の状態を保存し、画面で再試行可能にする
